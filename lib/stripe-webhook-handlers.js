...
export async function handleCheckoutSessionCompleted(session) {
  try {
    ...
    // For subscription mode, also fetch and update subscription information
    if (session.mode === "subscription" && session.subscription) {
      try {
        // Retrieve the full subscription data
        const subscription = await stripe.subscriptions.retrieve(
          session.subscription
        );

        if (subscription) {
          patchData.stripeSubscriptionId = subscription.id;
          patchData.subscriptionStatus = subscription.status;

          // Set current period end
          ...
          console.log(
            `Subscription data fetched and updated for: ${application.companyName}`
          );
        }
      } catch (subError) {
