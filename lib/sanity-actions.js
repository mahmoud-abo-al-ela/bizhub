...
export async function updateSubmissionStatus(submissionId, status) {
  try {
    ...
    // If approved, create company for all plans (free plans immediately, paid plans when payment is completed)
    if (status === "approved") {
      ...
      // For paid plans, create company but mark as pending payment
      else if (
...
export async function updatePaymentStatus(submissionId, paymentStatus) {
  try {
    ...
    // If payment is now paid, update the company document
    if (paymentStatus === "paid" && submission.status === "approved") {
