  ...
  try {
    ...
    const company = await backendClient.create({
      ...
      ...(submission.subscriptionStatus && {
        subscriptionStatus: submission.subscriptionStatus,
      }),
...
export async function updatePaymentStatus(submissionId, paymentStatus) {
  try {
    ...
    // If payment is now paid and company already exists, update the company document
      ...
      if (company) {
        // Get the latest submission data to ensure we have the most up-to-date subscription info
        const latestSubmission = await backendClient.getDocument(submissionId);
        ...
        // Copy subscription-related fields if they exist in the submission
        if (latestSubmission.stripeSubscriptionId) {
          updateData.stripeSubscriptionId =
            latestSubmission.stripeSubscriptionId;
        }

        if (latestSubmission.subscriptionStatus) {
          updateData.subscriptionStatus = latestSubmission.subscriptionStatus;
        }

        if (latestSubmission.currentPeriodEnd) {
          updateData.currentPeriodEnd = latestSubmission.currentPeriodEnd;
        }
